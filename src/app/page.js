...
const PROJECTS = [
  { title: 'RedBus',           description: 'UI/UX design focused on clean layouts and smooth user flow.', image: './Redbus.png',   tags: ['Tailwind CSS', 'HTML'] },
  { title: 'Zomato App Clone', description: 'UI/UX design focused on clean layouts and smooth user flow.', image: './Zomato.png',   tags: ['Figma', 'UI Design'] },
  { title: 'Travel Dashboard', description: 'UI/UX design focused on clean layouts and smooth user flow.', image: './Travel.png',   tags: ['Figma', 'UI Design'] },
  { title: 'Snapdeal Clone',   description: 'UI/UX design focused on clean layouts and smooth user flow.', image: './snapdeal.png', tags: ['Tailwind CSS', 'HTML'] },
  { title: 'Gozoop Social',    description: 'UI/UX design focused on clean layouts and smooth user flow.', image: './gozoop.png',   tags: ['Figma', 'UI Design'] },
];

const SKILLS = [
  { name: 'Tailwind CSS',      level: 75, Icon: Code2 },
  { name: 'HTML & CSS',        level: 80, Icon: Code2 },
  { name: 'WordPress',         level: 80, Icon: Palette },
  { name: 'JavaScript',        level: 80, Icon: Palette },
  { name: 'Figma',             level: 85, Icon: Code2 },
  { name: 'React & Next.js',   level: 25, Icon: Database },
  { name: 'UI/UX Design',      level: 89, Icon: Palette },
  { name: 'Node.js & Express', level: 20, Icon: Database },
];
...
const STYLES = `
  ...
  .card-hover { transition: transform .4s cubic-bezier(.22,.68,0,1.2), box-shadow .4s cubic-bezier(.22,.68,0,1.2); }
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900" style={{ fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <nav className="fixed top-0 w-full z-50" style={{ background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            {/* logo */}
            <button onClick={() => scrollTo('home')} className="text-2xl font-bold hover:scale-105 transition-transform" style={{ fontFamily: "'Syne',sans-serif" }}>
            ...
            <div className="hidden md:flex items-center gap-8">
              {NAV_LINKS.map((s) => (
                <button key={s} onClick={() => scrollTo(s)} className="capitalize font-medium relative" style={{ color: activeSection === s ? '#67e8f9' : '#94a3b8', transition: 'color .3s' }}>
                  {s}
          ...
          {isMenuOpen && (
            <div className="md:hidden mt-4 py-4" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              {NAV_LINKS.map((s) => (
                <button key={s} onClick={() => scrollTo(s)} className="block w-full text-left py-2 capitalize text-gray-400 hover:text-cyan-300 transition-colors">{s}</button>
              ))}
            </div>
          )}
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6" style={{ animation: 'revealUp .85s cubic-bezier(.22,.68,0,1.2) both' }}>
              ...
              <div className="flex flex-wrap gap-4 pt-2" style={{ animation: 'revealUp .7s .46s cubic-bezier(.22,.68,0,1.2) both' }}>
                ...
                <button onClick={() => scrollTo('contact')} className="btn-outline px-8 py-4 rounded-xl font-semibold relative z-10">
                  Contact Me
                </button>
              </div>
            ...
            <div className="relative flex items-center justify-center" style={{ animation: 'revealPop .8s .25s cubic-bezier(.22,.68,0,1.2) both' }}>
              ...
              <div className="relative z-10 w-72 h-72 rounded-2xl overflow-hidden" style={{ boxShadow: '0 8px 50px rgba(6,182,212,0.3)' }}>
                <img src="./farhan.png" alt="Farhan Tolkar" className="w-full h-full object-cover" style={{ transition: 'transform .5s cubic-bezier(.22,.68,0,1.2)' }}
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 mb-16">
            {ABOUT_CARDS.map(({ Icon, title, desc, accent }, i) => (
              <div key={title} className="p-8 bg-white rounded-2xl border border-gray-100"
                style={{
                  boxShadow: '0 2px 12px rgba(0,0,0,0.04)',
                  opacity: cardsVis ? 1 : 0,
                  transform: cardsVis ? 'translateY(0)' : 'translateY(38px)',
                  transitionProperty: 'opacity, transform, box-shadow',
                  transitionDuration: '.7s, .7s, .4s',
                  transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2), cubic-bezier(.22,.68,0,1.2), ease',
                  transitionDelay: cardsVis ? `${i * 120}ms` : '0ms',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = `0 14px 36px ${accent}25`; e.currentTarget.style.transform = 'translateY(-7px)'; }}
              ...
              <div className="relative z-10 grid md:grid-cols-2 gap-8 items-center">
                ...
                <div className="space-y-4">
                  {JOURNEY_ITEMS.map((item, i) => (
                    <div key={item} className="flex items-center gap-3" style={{ color: '#94a3b8', opacity: journeyVis ? 1 : 0, transform: journeyVis ? 'translateX(0)' : 'translateX(24px)', transitionProperty: 'opacity, transform', transitionDuration: '.6s', transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2)', transitionDelay: `${i * 100}ms` }}>
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {PROJECTS.map((project, i) => (
              <div key={i} className="group bg-white rounded-2xl overflow-hidden border border-gray-100"
                style={{
                ...
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 18px 44px rgba(6,182,212,0.16)'; e.currentTarget.style.transform = 'translateY(-7px)'; }}
      ...
      <section id="skills" className="py-24 px-6 bg-white">
        <div className="container mx-auto max-w-6xl">
          {/* heading */}
          <div className="text-center mb-16" style={{ opacity: skillsVis ? 1 : 0, transform: skillsVis ? 'translateY(0)' : 'translateY(38px)', transitionProperty: 'opacity, transform', transitionDuration: '.7s', transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2)' }}>
            <h2 className="text-4xl md:text-5xl font-bold mb-4" style={{ fontFamily: "'Syne',sans-serif" }}>
            ...
            {SKILLS.map((skill, i) => (
              <div key={i} className="p-5 bg-white rounded-xl border border-gray-100"
                style={{
                  ...
                  transitionProperty: 'opacity, transform, box-shadow',
                  transitionDuration: '.65s, .65s, .4s',
                  transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2), cubic-bezier(.22,.68,0,1.2), ease',
                  transitionDelay: `${i * 90}ms`,
                }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 8px 24px rgba(6,182,212,0.14)'; e.currentTarget.style.transform = 'translateY(-7px)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.04)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div style={{ color: '#0891b2' }}><skill.Icon className="w-5 h-5" /></div>
          ...
          <div ref={featsRef} className="grid md:grid-cols-3 gap-6">
            {FEATURE_CARDS.map(({ emoji, title, desc }, i) => (
              <div key={title} className="text-center p-8 rounded-2xl border border-gray-100"
                style={{
                ...
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 12px 30px rgba(6,182,212,0.15)'; e.currentTarget.style.transform = 'translateY(-7px)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.04)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
      ...
      <section id="contact" className="py-24 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#0c1220 0%,#0f172a 40%,#1a2332 100%)' }}>
        ...
        <div ref={contactRef} className="container mx-auto max-w-4xl relative z-10">
          ...
          <div style={{ opacity: contactVis ? 1 : 0, transform: contactVis ? 'translateY(0)' : 'translateY(38px)', transitionProperty: 'opacity, transform', transitionDuration: '.72s', transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2)', transitionDelay: '150ms' }}>
            <div className="rounded-2xl p-8 md:p-10" style={{ background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {CONTACT_INFO.map(({ Icon, label, value }, i) => (
                  <div key={label} className="flex items-center gap-4" style={{ opacity: contactVis ? 1 : 0, transform: contactVis ? 'translateY(0)' : 'translateY(20px)', transitionProperty: 'opacity, transform', transitionDuration: '.6s', transitionTimingFunction: 'cubic-bezier(.22,.68,0,1.2)', transitionDelay: `${200 + i * 120}ms` }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-6">
              {['Home', 'About', 'Projects', 'Skills', 'Contact'].map((item) => (
                <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="footer-link text-xs">{item}</button>
              ))}
            </div>
            <div className="flex items-center gap-3">
              {SOCIAL_LINKS.map(({ href, Icon }) => (
